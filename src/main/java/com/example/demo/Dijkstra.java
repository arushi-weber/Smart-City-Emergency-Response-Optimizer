...
public class Dijkstra extends Citygraphh {
    ...
    // ✅ New method to get shortest path (IMPORTANT for API)
    public static List<Integer> getPath(int src, int dest) {

        loadGraph(); // ensure graph is loaded

        Result res = dk(location, adj, src);

        List<Integer> path = new ArrayList<>();

        if (res.dist[dest] == (int) 1e9) {
